...
import InformeVentaVisita from "./InformeVentaVisita.jsx";
              ...
              <tbody className="bg-white divide-y divide-gray-200 font-medium text-center items-center">
                {datosSimulados.map((venta, index) => (
                  <tr key={venta.id} className="hover:bg-gray-100">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-700">
                      {format(new Date(venta.fechaCompra), "hh:mm a", {
                        locale: es,
                      })}
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-700">{formatoPrecio.format(venta.precio)}</td>
                    
                    <td className="px-6 flex gap-3 py-4 text-sm text-gray-700 justify-center">
                      <button
                      ...
                      {activeModal === venta.id && (
                        <InformeVentaVisita closeModal={closeModal} />
                      )}
                      <button
