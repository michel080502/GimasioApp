	...
	<>
		<div className="my-4 p-3 bg-white rounded-lg">
			<div className="p-2 grid grid-cols-4 gap-5">
				...
				<div className="grid grid-cols-2 gap-3  items-center text-lg">
					<button className=" border-r-2 p-4 border-gray-900 h-8 flex gap-7 items-center hover:bg-zinc-600 hover:bg-opacity-20"> <RiFileExcel2Fill  /> Exportar</button>
					<button className=" p-4 flex h-8 gap-7 items-center hover:bg-zinc-600 hover:bg-opacity-20"><FaFilter />
					Filtro</button>
				</div>
			</div>
				...
				<tbody className="bg-white divide-y divide-gray-200 text-center items-center">
					{clientes.map((cliente, index) => (
						<tr key={cliente._id} className="hover:bg-gray-100 ">
							...
							<td className="px-6 py-4 text-sm text-gray-700 "><img className="w-16 rounded-full ring-2 ring-red-800 m-auto" src={cliente.img.secure_url} alt="profile" /></td>
							<td className="px-6 py-4 text-sm font-semibold text-gray-700">{cliente.nombre} {cliente.apellidoPaterno} {cliente.apellidoMaterno}</td>
							<td className="px-6 py-4 text-sm font-semibold text-gray-700">{cliente.matricula}</td>
							<td className="px-6 py-4 text-sm font-semibold text-gray-700">{cliente.telefono}</td>
									...
									{deleteCliente === cliente._id && (
										<div className="absolute  mt-2 bg-white border rounded-lg shadow-lg w-56 z-10 p-4 ">
											...
											<span className='font-bold'>
												{cliente.nombre} {cliente.apellidoPaterno}</span>?
											</h1>
